...
author: "Zachary Wood"
date: "9/23/26"
image: images/lab3_top_level_block_diagram.png
categories:
  - labreport
draft: false
---
...
![Figure 4: Block diagram of the `keypad_scanner` SystemVerilog module.](images/lab3_keypad_scanner_block_diagram.png)

Figure 5 shows `debouncer`. The FSM's next state logic uses `btn_in` and bit 19 of `db_count`. `db_clr`, the OR of `rst` and an equality comparison of `state` against IDLE, resets `db_counter`, whose enable is tied high, and `btn_out` is the inverted equality comparison of `state` against PRESSED.

![Figure 5: Block diagram of the `debouncer` SystemVerilog module.](images/lab3_decouncer_block_diagram.png)
...
![Figure 6: Schematic of the physical circuit.](images/lab3_schematic.png)
...
## Synthesis output (click to expand)
![Figure 7: Lattice Synthesis Engine area report for `lab3_zw`.](images/lab3_synthesis_output.png)
:::
...
Figures 8 through 10 show the synchronizer, debouncer, and keypad scanner testbenches. Figure 11 shows the top-level testbench, and Figure 12 zooms in on the end of the top-level simulation. The counter and seven-segment decoder simulations were not repeated, since those modules were verified in [lab 1](../lab1/lab1.qmd).

::: {.callout-note collapse="true"}
## Synchronizer testbench (click to expand)
![Figure 8: QuestaSim waveform for `synchronizer_tb`, showing the two-cycle latency for rising, falling, and edge-aligned inputs, and an asynchronous reset in the middle of a shift.](images/lab3_synchronizer_tb_waveform.png)
:::

::: {.callout-note collapse="true"}
## Debouncer testbench (click to expand)
![Figure 9: QuestaSim waveform for `debouncer_tb`, showing the IDLE, WAIT, and PRESSED transitions, a bounce returning to IDLE, confirmation at the debounce threshold, recovery from the unused state, and an asynchronous reset.](images/lab3_debouncer_tb_waveform.png)
:::

::: {.callout-note collapse="true"}
## Keypad scanner testbench (click to expand)
![Figure 10: QuestaSim waveform for `keypad_scanner_tb`, showing the row boundaries, all 16 decodes, a long hold, and the multiple-key cases.](images/lab3_keypad_scanner_tb_waveform.png)
:::

![Figure 11: QuestaSim waveform for `lab3_zw_tb`, showing a bounced, asynchronous press of key 1 propagating to `seg`, followed by a press of key 2.](images/lab3_top_tb_waveform.png)

::: {.callout-note collapse="true"}
## Top-level testbench, zoomed (click to expand)
![Figure 12: Zoomed view of `lab3_zw_tb`, showing key 2 reaching `seg` during digit 0's phase, followed by the asynchronous reset.](images/lab3_top_tb_waveform_zoomed.png)
:::
